...
$patientEmail_err = $patientPassword_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty(trim($_POST["patientEmail"]))){
        $patientEmail_err = "Please enter email.";
    } else{
    ...
    if(empty(trim($_POST["patientPassword"]))){
        ...
        $password = trim($_POST["patientPassword"]);
    }
    
    // Validate credentials
    if(empty($patientEmail_err) && empty($patientPassword_err)){
        // Prepare a select statement
        $sql = "SELECT patientEmail, patientEmail FROM patient WHERE patientEmail = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $patientEmail, $patientPassword);
                    if(mysqli_stmt_fetch($stmt)){
                            ...
                            $patientPassword_err = "The password you entered was not valid.";
                        }
                    }
                } else{
                    // Display an error message if username doesn't exist
                    $patientEmail_err = "No account found with that username.";
                }
